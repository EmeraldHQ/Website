...
function updateConfig() {
    ...
    const sdkConfigVersion = inlangConfig.split("\n")
        .find(line => line.includes("sdk-js-plugin"))
        .split("sdk-js-plugin")[1]
        .split("@")[1].split("/")[0];

    console.log(`Installed inlang sdk version: ${sdkDependencyVersion} - config version: ${sdkConfigVersion}`);

    // If versions are different, update config
    if (sdkDependencyVersion !== sdkConfigVersion) {
        console.log("Updating inlang config...");
        const newConfig = inlangConfig.replace(/sdk-js-plugin[^@]+@[^/]+/, `sdk-js-plugin@${sdkDependencyVersion}`);
